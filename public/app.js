...
function toggleCompare(id){const n=Number(id);if(!Number.isFinite(n))return;if(isCompared(n)){state.compare=state.compare.filter(x=>Number(x)!==n);saveCompare()}else{if(state.compare.length>=3)return toast('حداکثر ۳ محصول را می‌توانید مقایسه کنید');state.compare.push(n);saveCompare()}}
