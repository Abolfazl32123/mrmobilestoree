...
async function listProducts(env,all=false){
  ...
  const optional=['discount_price','badge','featured','bestseller','newest','category','specs','stock_qty','low_stock_threshold'];
  ...
  return rows.map(p=>({
    discount_price:p.discount_price??'',badge:p.badge??'',featured:Number(p.featured||0),bestseller:Number(p.bestseller||0),newest:Number(p.newest||0),category:p.category||'موبایل',specs:p.specs||'{}',stock_qty:Number(p.stock_qty??(p.available?1:0)),low_stock_threshold:Number(p.low_stock_threshold??2),...p
  }));
}
...
export default {async fetch(request,env){
  ...
  try{
    ...
    if(path==='/api/orders'&&request.method==='POST'){
      ...
      let subtotal=0,clean=[];for(const it of items){const p=await env.DB.prepare('SELECT id,price,available,COALESCE(stock_qty,CASE WHEN available=1 THEN 1 ELSE 0 END) AS stock_qty FROM products WHERE id=?').bind(Number(it.product_id)).first();if(!p||!p.available||Number(p.stock_qty)<=0)continue;const price=Number(String(p.price).replace(/[^\d]/g,''))||0,q=Math.max(1,Math.min(99,Number(it.quantity)||1));if(q>Number(p.stock_qty))return json({error:`موجودی «محصول» کافی نیست. حداکثر ${Number(p.stock_qty)} عدد موجود است.`},409);subtotal+=price*q;clean.push({id:p.id,price,q})}
