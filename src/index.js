...
async function listProducts(env,all=false){
  ...
  if(!all)q+=' WHERE COALESCE(stock_qty,CASE WHEN available=1 THEN 1 ELSE 0 END)>0';
  q+=' ORDER BY id DESC';
  const rows=(await env.DB.prepare(q).all()).results||[];
  return rows.map(p=>{
    const qty=Math.max(0,Number(p.stock_qty??(p.available?1:0))||0);
    return {...p,discount_price:p.discount_price??'',badge:p.badge??'',featured:Number(p.featured||0),bestseller:Number(p.bestseller||0),newest:Number(p.newest||0),category:p.category||'موبایل',specs:p.specs||'{}',stock_qty:qty,quantity:qty,available:qty>0,low_stock_threshold:Number(p.low_stock_threshold??2),sale_start_at:p.sale_start_at??null,sale_end_at:p.sale_end_at??null};
  });
}
