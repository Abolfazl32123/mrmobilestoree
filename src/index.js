...
async function listProducts(env,all=false){
  ...
  // Public catalog includes out-of-stock products too; the UI marks them as ناموجود.
  // Admin mode still returns all products.

  q+=' ORDER BY id DESC';
  const rows=(await env.DB.prepare(q).all()).results||[];
  return rows.map(p=>({
    discount_price:p.discount_price??'',badge:p.badge??'',featured:Number(p.featured||0),bestseller:Number(p.bestseller||0),newest:Number(p.newest||0),category:p.category||'موبایل',specs:p.specs||'{}',stock_qty:Number(p.stock_qty??(p.available?1:0)),low_stock_threshold:Number(p.low_stock_threshold??2),sale_start_at:p.sale_start_at??null,sale_end_at:p.sale_end_at??null,...p
  }));
}
...
export default {async fetch(request,env){
  ...
  try{
    ...
    if(path==='/api/account/advanced'&&request.method==='GET'){const uid=await userFromReq(request,env);if(!uid)return json({error:'Unauthorized'},401);await ensureAdvancedTables(env);await env.DB.prepare('INSERT OR IGNORE INTO customer_wallets(user_id) VALUES(?)').bind(uid).run();await env.DB.prepare('INSERT OR IGNORE INTO loyalty_points(user_id) VALUES(?)').bind(uid).run();let ref=await env.DB.prepare('SELECT * FROM referrals WHERE user_id=?').bind(uid).first();if(!ref){const code='MR'+uid+Math.random().toString(36).slice(2,7).toUpperCase();await env.DB.prepare('INSERT OR IGNORE INTO referrals(user_id,code) VALUES(?,?)').bind(uid,code).run();ref=await env.DB.prepare('SELECT * FROM referrals WHERE user_id=?').bind(uid).first()}const wallet=await env.DB.prepare('SELECT balance FROM customer_wallets WHERE user_id=?').bind(uid).first();const points=await env.DB.prepare('SELECT points FROM loyalty_points WHERE user_id=?').bind(uid).first();return json({wallet:Number(wallet?.balance||0),points:Number(points?.points||0),referral_code:ref?.code||''})}
    ...
    if(path==='/api/catalog'&&request.method==='GET'){try{await ensureProductColumns(env);const rows=await listProducts(env,false);return json(rows)}catch(e){return json({error:'خطا در دریافت فهرست محصولات',detail:String(e?.message||e)},500)}}
